---
title: "Determinants of Health Outcomes, A Multivariate Analysis"
...
```{r download_data}
write.csv(full_data, "Full Dataset.csv")
```
...
```{r healthspendinglinegraph, warning =FALSE, echo=FALSE, fig.height = 3}
full_data %>%
...
ggplot(aes(x=Year, y=log(the_per_cap_ppp_mean), group = `Country Name`)) +
 ...
 labs( 
   title = "Health Spending Around the World Over Time",
       y = "Log of Mean THE per Capita (log PPP dollars)") +
  theme(axis.title.y = element_text(size = 8))
```
...
\
```{r healthspending_and_lifeexpectancymap, warning =FALSE, echo=FALSE, fig.width=10, fig.height= 8}
...
grid.arrange(grobs = list(map1, map2, map3), nrow = 3, top = "Geographic Trends of Health Measures and Health Expenditure")
```
...
ggplot(aes(x = log(the_per_cap_ppp_mean), y = `Mortality rate, under-5 (per 1,000 live births)`)) + 
  geom_point(alpha = 0.6) +
  labs(y = "Mortality Rates Under 5 (per 1000)",
       x = NULL)  +
  theme(axis.title.y = element_text(size = 8))

grid.arrange(grobs = list(THE_lifeexp, THE_mort), ncol = 2, top = "Correlation Between Health Expenditure and Health Outcomes", bottom = "Log of Mean THE per Capita (in PPP dollars)")
```
...
le_resid1 <- ggplot(lifeexp_fullfit_aug, mapping = aes(x = .fitted, y = .std.resid)) +
...
le_resid2 <- ggplot(lifeexp_fit_aug, mapping = aes(x = .fitted, y = .std.resid)) +
  geom_point(alpha = 0.5) +
  geom_hline(yintercept = 0, color = "blue", lty = "dashed") +
...
Given that this model has the highest adjusted R$^2$ value, we have decided to interpret the coefficient estimates and their significance. Based on the results above, log(THE) does not have a significant linear relationship to total life expectancy (p-value = 0.4004 > 0.05) and does have a significant quadratic relationship to total life expectancy (p-value = 0.0868 > 0.05), holding all other covariates constant. log(GDP) does not have a significant linear relationship to total life expectancy (p-value = 0.0997 > 0.05), holding all other covariates constant. Poverty does not have a significant linear relationship to total life expectancy (p-value = 0.4171 > 0.05), holding all other covariates constant.

Education does have a significant linear relationship (p-value $\approx$ 0.0000 < 0.05) and significant quadratic relationship (p-value $\approx$ 0.0000 < 0.05) to total life expectancy, holding all other covariates constant. For every one increase in the percentage of the population 25+ that has at least completed upper secondary education, the total life expectancy of a country is expected to increase by 0.4655 years, holding all other covariates constant. Moreover, for every one increase in the percentage of the population 25+ that has at least completed upper secondary education squared, the total life expectancy of a country is expected to decrease by 0.0025, holding all other covariates constant. This indicates that increasing educational attainment in a country is expected to decreasing margin returns on increasing total life expectancy. 

GDP growth does have a significant linear relationship to total life expectancy (p-value = 0.0163 < 0.05), holding all covariates constant. For every one percentage increase in GDP growth, the total life expectancy of a country is expected to decrease by 0.3134, holding all other covariates constant. 

The interaction term between poverty and education does have a significant linear relationship to total life expectancy (p-value $\approx$ 0.0000 < 0.05). Since -0.0050 < 0, the estimated effect of poverty and education on total life expectancy are negatively related. The interaction term between GDP growth and education does have a significant linear relationship to total life expectancy (p-value = 0.0094 < 0.05). Given that 0.0059 > 0, the estimated effect of GDP growth and education on total life expectancy are positively related. The interaction term between poverty and GDP does have a significant linear relationship to total life expectancy (p-value = 0.0409 < 0.05). Because -0.0222 < 0, the estimated effect of poverty and GDP on total life expectancy are negatively related.  
...
le_resid3 <- ggplot(lifeexp_mixedfit_aug, mapping = aes(x = .fitted, y = .std.resid)) +
  geom_point(alpha = 0.5) +
  geom_hline(yintercept = 0, color = "blue", lty = "dashed") +
...
```{r resid_plots, fig.height = 3}
grid.arrange(grobs = list(le_resid1, le_resid2, le_resid3), ncol = 3,
             top = "Life Expectancy Regression Model Residual Plots",
             bottom = "Predicted Life Expectancy (years)")
```
...
mort_resid1 <- ggplot(mortality_fullfit_aug, mapping = aes(x = .fitted, y = .std.resid)) +
  geom_point(alpha = 0.5) +
  geom_hline(yintercept = 0, color = "blue", lty = "dashed") +
  labs(title = "Simple Linear Model",
       x = NULL,
...
mort_resid2 <- ggplot(mortality_fit_aug, mapping = aes(x = .fitted, y = .std.resid)) +
  geom_point(alpha = 0.5) +
  geom_hline(yintercept = 0, color = "blue", lty = "dashed") +
  labs(title = "Quadratic Model",
       x = NULL,
...
Similar to our approach for the life expectancy regression models, we are focusing our efforts on interpreting the model with the greatest R$^2$ value. Out of the 19 association terms, 13 are significant (p < 0.05, excluding $\beta_0$). Although there is no significant linear relationship between log(THE) and mortality rates ($\beta_1$, p = 0.793), there is a significant quadratic relationship ($\beta_2$, p < 0.05). Holding all other covariates constant, for every 1 increase in log(THE)$^2$, we expect, on average, an 0.120 increase in under-5 mortality rate. This is also the case with log(GDP), where there is no significant linear relationship (p = 0.183), but there is a significant quadratic relationship ($\beta_2$, p < 0.05). Holding all other covariates constant, for every 1 increase in log(GDP)$^2$, we expect, on average, an 0.041 increase in under-5 mortality rate. Income is another variable that shares this affect on mortality rate. The linear term is not significant ($\beta_5$, p = 0.758) while the quadratic term is ($\beta_6$, p < 0.05). Holding all other covariates constant, for every 1 increase in income, we expect, on average, an 0.041 increase in under-5 mortality rate. Income is another variable that shares this affect on mortality rate. For every 1 increase
...
mort_resid3 <- ggplot(mortality_mixedfit_aug, mapping = aes(x = .fitted, y = .std.resid)) +
  geom_point(alpha = 0.5) +
  geom_hline(yintercept = 0, color = "blue", lty = "dashed") +
  labs(title = "Quadratic Interaction Model",
       x = NULL,
...
```{r mort_resid_plots, fig.height = 3}
grid.arrange(grobs = list(mort_resid1, mort_resid2, mort_resid3), ncol = 3,
             top = "Mortality Rate Regression Model Residuals",
             bottom = "Predicted Mortality Rate Under-5 (per 1000)",
             left = "Residuals")
```
