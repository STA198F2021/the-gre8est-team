...
lifeexp_fit <- linear_reg() %>%
  set_engine("lm") %>%
  fit(`Total Life Expectancy` ~ log(the_per_cap_ppp_mean) + `GDP (current US$)` + `GDP growth (annual %)` + `Income share held by lowest 20%` + `Poverty headcount ratio at national poverty lines (% of population)`, data = full_data_2)
