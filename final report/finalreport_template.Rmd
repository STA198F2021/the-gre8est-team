...
result1 <- t(result1)
rownames(result1) <- c("Term", "Value", "p-value", "CI Lower", "CI Higher")
result1 %>% kable(row.names = ,
                  escape = FALSE,
                  digits = 6) %>%
...
result6$term <- vals6
result6 <- result6[-c(3:4)]
result6 %>% kable(col.names = c("Term", "Value", "p-value", "CI Lower", "CI Higher"),
                  escape = FALSE,
                  digits = 6) %>%
  kable_classic(full_width = F, html_font = "helvetica") %>%
  kable_styling(latex_options = "hold_position")
