...
```{r outcome-histo, warning = FALSE, echo=FALSE}
p_lifeexp <- full_data %>%
...
p_log_pov <- full_data %>%
  ...
  labs(
    ...
    y = "Frequency",
    title = "Distribution of Poverty Headcount"
  )
...
p_inc <- full_data %>%
  ...
  labs(
    ...
    y = "Frequency",
    title = "Distribution of Income Share"
  )
...
p_edu <- ggplot(data = full_data, aes(full_data$`Educational attainment, at least completed upper secondary, population 25+, total (%) (cumulative)`)) + 
  ...
  labs(
    ...
    y = "Frequency",
    title = "Distribution of Secondary Educational Attainment"
  )

grid.arrange(grobs = list(p_log_the, p_log_gdp, p_gdpg, p_pov, p_inc, p_edu), nrows = 3)
```
...
labs( x= "Log of Mean Total Health Spending per Person (in PPP dollars)",
      y = "Total Life Expectancy",
      title = "Mean Total Health Spending per Person versus Life Expectancy")
